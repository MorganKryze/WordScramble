namespace WordScramble.models
{
    ...
    public class Board
    {
        ...
        public Board(string boardDifficulty)
        {
            ...
            CustomDictionary temporaryDictionary = new CustomDictionary();
            Dictionary<string, int> config = Methods.ConfigurationJson("Settings.json",BoardDifficulty);
            Timer = config["timer"];
        ...
        public bool TryPlaceColumn(Position pos, string word)
        {
            ...
            foreach(int i in Enumerable.Range(1, 2).OrderBy(x => Methods.rnd.Next()))
            {
                switch(i)
                {
                    case 1 : 
            ...
            foreach(int i in Enumerable.Range(1, 2).OrderBy(x => Methods.rnd.Next()))
            {
                switch(i)
                {
                    case 1:
                        ...
                        else if (IsFreeToFill(diagonal1, word.Reverse().ToString()?? throw new NullReferenceException())) return FillWord(word.Reverse().ToString()?? throw new NullReferenceException(), pos, new Position(pos.X - word.Length, pos.Y + word.Length));
                        break;
                    case 2: 
                        if(IsFreeToFill(diagonal2, word)) return FillWord(word, pos, new Position(pos.X + word.Length, pos.Y - word.Length));
                        else if(IsFreeToFill(diagonal2, word.Reverse().ToString()?? throw new NullReferenceException())) return FillWord(word.Reverse().ToString()?? throw new NullReferenceException(), pos, new Position(pos.X + word.Length, pos.Y - word.Length));
                        break;
                }
            }
            return false;
        ...
        {
            int[] type = Enumerable.Range(1, numberOfDirections*2).OrderBy(x => Methods.rnd.Next()).Take(numberOfDirections*2).ToArray();
        ...
        public List<string> PlaceRandomWords(CustomDictionary dictionary, int difficulty, List<int> amountToPlace, List<string>? successfullyPlaced = null)
        {
            successfullyPlaced ??= new List<string>();
            for(int i = amountToPlace.Count-1; i >= 0; i--)
            {
                for(int j = 0; j < amountToPlace[i]; j++)
                {
                    ...
                    while(!isPlaced)
                    {
                        ...
                        int n = Methods.rnd.Next((i+1) * 3, (i+1) * 3 + 3);
                        string word = dictionary.ListDict[n][Methods.rnd.Next(0, dictionary.ListDict[n].Count)];
        ...
        public Position RandomPosition() => new Position(Methods.rnd.Next(0, Matrix.GetLength(0)), Methods.rnd.Next(0, Matrix.GetLength(1)));
        ...
        public void FillMatrix(bool fillWithLetters = false)
        {
            if(fillWithLetters)
            {
                for(int i = 0; i < Matrix.GetLength(0); i++)
                {
                    ...
                    {
                        if(Matrix[i, j] == '_') Matrix[i, j] = (char)Methods.rnd.Next(65, 91);
                    }
                }
            } 
            else for(int i = 0; i < Matrix.GetLength(0); i++) for(int j = 0; j < Matrix.GetLength(1); j++) Matrix[i, j] = '_';
        }
        /// <summary>This method is used to display the matrix into the console.</summary>
